...
const links = [{ label: "Solutions", href: "#solutions" }, { label: "Our approach", href: "#approach" }, { label: "About us", href: "#about" }, { label: "Insights", href: "#insights" }];
  ...
  return (
    <header className={styles.header}>
      ...
      <nav className={`${styles.container} ${styles.nav}`} aria-label="Main">
        ...
        <a className={styles.navContact} href={site.contact}>Let’s talk <Arrow /></a>
      ...
      <nav id="mobile-nav" aria-label="Mobile" hidden={!open} className={styles.mobileNav}>{links.map(link => <a key={link.href} href={link.href} onClick={() => setOpen(false)}>{link.label}<Arrow /></a>)}<a href={site.contact}>Let’s talk <Arrow /></a></nav>
    </header>
  );
}
